...
```
```{r}
try_random= mppm(cell ~ 1, hyoertime, random= ~1 | time)
```

```{r}
try_pp=as.ppp(D4_ty2$x, D4_ty2$y, d4_owin, marks=fitted_intensities$trend[[1]])
```


```{r}
K_functions <-  Kinhom(fitted_intensities2, ratio= TRUE)
```
